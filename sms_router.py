...
La cola SMS es persistente en PostgreSQL/Neon. El envío móvil se ejecuta con
Worker Pull en Android/Termux, protegido por Bearer token. La auditoría de
cada intento queda en gestiones_crm y es idempotente.
"""
...
from fastapi import APIRouter, BackgroundTasks, Form, Header, HTTPException, Query, Request, status
...
ROLES_FORZAR_HORARIO = {"admin", "administrador", "abogado_director", "director", "superadmin"}


def _get_api_token() -> str:
    token = os.getenv("SMS_API_TOKEN")
    if not token or not token.strip():
        raise RuntimeError("CRÍTICO: La variable de entorno 'SMS_API_TOKEN' no está configurada en Render.")
    return token.strip()
...
def _validar_permiso_forzar_horario(request: Request) -> None:
    ...
    if not user_id:
        raise HTTPException(status_code=403, detail="No existe una sesión de usuario válida.")
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                ...
                ORDER BY CASE column_name
                    WHEN 'rol' THEN 1 WHEN 'role' THEN 2 WHEN 'perfil' THEN 3
                    WHEN 'cargo' THEN 4 WHEN 'tipo_usuario' THEN 5 ELSE 99 END
                LIMIT 1;
                """
            )
            columna = cur.fetchone()
            if not columna:
                raise HTTPException(status_code=403, detail="No existe un campo de rol/perfil autorizado en la tabla de abogados.")
            ...
            rol = str((usuario or {}).get("rol") or "").strip().lower()
            if rol not in ROLES_FORZAR_HORARIO:
                raise HTTPException(status_code=403, detail="Solo el Abogado Director o Administrador pueden forzar envíos fuera del horario legal.")
    finally:
        conn.release()
    ...
    try:
        with conn:
            with conn.cursor() as cur:
                ...
                cur.execute("ALTER TABLE sms_cola_envios ADD COLUMN IF NOT EXISTS contacto_id INT REFERENCES contactos(id) ON DELETE SET NULL;")
                cur.execute("ALTER TABLE sms_cola_envios ADD COLUMN IF NOT EXISTS fecha_proceso TIMESTAMP;")
                cur.execute("ALTER TABLE sms_cola_envios ADD COLUMN IF NOT EXISTS processing_token VARCHAR(128);")
                cur.execute("ALTER TABLE sms_cola_envios ADD COLUMN IF NOT EXISTS crm_auditado BOOLEAN NOT NULL DEFAULT FALSE;")
                cur.execute("ALTER TABLE sms_cola_envios ADD COLUMN IF NOT EXISTS crm_auditoria_fecha TIMESTAMP;")
                cur.execute(
                    """
                    UPDATE sms_cola_envios s
                    SET contacto_id=i.contacto_id
                    FROM inmuebles_ph i
                    WHERE s.inmueble_id=i.id AND s.contacto_id IS NULL;
                    """
                )
                cur.execute(
                    """
                ...
                cur.execute("CREATE INDEX IF NOT EXISTS idx_sms_cola_estado ON sms_cola_envios(estado);")
                cur.execute("CREATE INDEX IF NOT EXISTS idx_sms_cola_proceso ON sms_cola_envios(estado, fecha_proceso);")
                cur.execute("CREATE INDEX IF NOT EXISTS idx_sms_cola_telefono ON sms_cola_envios(telefono);")
                cur.execute(
                    """
        ...
        raise RuntimeError(f"[SMS SCHEMA ERROR] Falló la migración del esquema SMS en Neon: {exc}") from exc
    finally:
        conn.release()
...
def normalizar_telefono(raw_tel: str) -> Optional[str]:
    if not raw_tel:
        return None
    partes = str(raw_tel).replace("/", " ").replace("-", " ").replace(",", " ").replace(";", " ").split()
    for parte in partes:
...
def _candidatos_cartera(
    ...
    """Devuelve candidatos por contacto, conservando soporte para cotitulares."""
    ...
    if ids:
        # IMPORTANTE: la UI selecciona contacto_id, no inmueble_id.
    ...
    if tiene_tabla_multiple:
        propietarios_cte = """
            propietarios AS (
                SELECT ip.inmueble_id, ip.contacto_id FROM inmueble_propietarios ip
                UNION ALL
                SELECT i.id, i.contacto_id
                FROM inmuebles_ph i
                WHERE NOT EXISTS (
                    SELECT 1 FROM inmueble_propietarios ip2 WHERE ip2.inmueble_id=i.id
                )
            )
        """
    else:
        propietarios_cte = """
            propietarios AS (
                SELECT i.id AS inmueble_id, i.contacto_id FROM inmuebles_ph i
            )
        """
    ...
    query = f"""
        ...
        saldos AS (
            SELECT inmueble_id,
                   SUM(CASE WHEN LOWER(COALESCE(concepto,''))!='abono'
                            THEN valor_capital ELSE -valor_capital END) AS saldo_total
            FROM expensas_ph
            GROUP BY inmueble_id
            HAVING SUM(CASE WHEN LOWER(COALESCE(concepto,''))!='abono'
                            THEN valor_capital ELSE -valor_capital END) > 0
        )
...
def _registrar_en_crm_idempotente(
    ...
    cur.execute("SELECT crm_auditado FROM sms_cola_envios WHERE id=%s FOR UPDATE;", (item_id,))
    fila = cur.fetchone()
    if not fila:
        raise RuntimeError(f"No existe sms_cola_envios.id={item_id}.")
    auditado = fila[0] if not isinstance(fila, dict) else fila.get("crm_auditado")
    if auditado:
        return False
    ...
    texto_col = next((col for col in ("observaciones", "resumen", "descripcion") if col in cols), None)
    if not texto_col:
        raise RuntimeError("gestiones_crm no posee una columna textual compatible (observaciones/resumen/descripcion).")

    marker = f"[SMS_ID:{item_id}]"
    cur.execute(f"SELECT 1 FROM gestiones_crm WHERE {texto_col} LIKE %s LIMIT 1;", (f"{marker}%",))
    if cur.fetchone():
        cur.execute("UPDATE sms_cola_envios SET crm_auditado=TRUE, crm_auditoria_fecha=CURRENT_TIMESTAMP WHERE id=%s;", (item_id,))
        return False
    ...
    tipo_gestion = "COBRANZA_JURIDICA" if tipo_campana in ("COBRO_JURIDICO", "MANDAMIENTO") else "COBRANZA_PREJUDICIAL"
    if "tipo_gestion" in cols:
        data["tipo_gestion"] = tipo_gestion
    elif "tipo" in cols:
        data["tipo"] = tipo_gestion

    resultado = "ENVIADO_OPERADOR" if estado == "ENVIADO" else f"FALLIDO: {error_detalle[:100]}"
...
def _asentar_resultado_sms_y_crm(
            ...
            with conn_sms.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute(
                    ...
                    SET estado=%s, fecha_envio=CURRENT_TIMESTAMP, error_detalle=%s,
                        fecha_proceso=NULL, processing_token=NULL
                    WHERE id=%s AND estado='EN_PROCESO' AND processing_token=%s
                    RETURNING id,inmueble_id,contacto_id,telefono,mensaje_texto,tipo_campana,saldo_calculado;
                    """,
                    (estado_final, str(respuesta_gateway)[:200], item_id, processing_token),
                )
                item = cur.fetchone()
                if not item:
                    return False
    except Exception as exc:
        print(f"[ERROR CRÍTICO] No fue posible asentar SMS id={item_id}: {exc}", flush=True)
        return False
    finally:
        conn_sms.release()
    ...
    try:
        ...
        print(f"[ALERTA AUDITORÍA CRM] SMS id={item_id} asentado como {estado_final}, pero CRM falló: {exc_crm}", flush=True)
...
def _reclamar_lote(cur, limite: int):
    """Reclama mensajes de forma exclusiva. Compatible con cursor tuple y RealDictCursor."""
    cur.execute(
        """
        SELECT id
    ...
    for fila in filas:
        ...
        cur.execute(
            ...
            RETURNING id,inmueble_id,contacto_id,telefono,mensaje_texto,tipo_campana,saldo_calculado,processing_token;
            """,
            (token, fila_id),
        )
        row = cur.fetchone()
...
def enviar_sms_gateway(telefono_10_digitos: str, mensaje: str) -> Tuple[bool, str]:
    ...
    try:
        response = __import__("requests").post(
            ANDROID_GATEWAY_URL,
            json={"to": f"+57{telefono_10_digitos}", "message": mensaje},
            timeout=12,
        )
        ...
        return bool(data.get("success") or data.get("status") == "sent" or "successfully" in texto.lower()), texto
    except Exception as exc:
...
def _tarea_despacho_background(limite: int = 50, dry_run: bool = False):
    ...
    for item in pendientes:
        ...
        _asentar_resultado_sms_y_crm(
            item_id=item["id"],
            processing_token=item["processing_token"],
...
def _autenticar_bearer(authorization: Optional[str] = Header(None)) -> None:
    ...
    if not authorization or not authorization.startswith("Bearer "):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Se requiere Authorization: Bearer <TOKEN>.", headers={"WWW-Authenticate": "Bearer"})
    token_recibido = authorization[7:].strip()
    if not token_recibido or not secrets.compare_digest(token_recibido, token_esperado):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Token de acceso inválido.", headers={"WWW-Authenticate": "Bearer"})
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                ...
                SELECT COUNT(*) FILTER (WHERE estado='PENDIENTE') AS pendientes,
                       ...
                       COALESCE(SUM(saldo_calculado) FILTER (WHERE estado IN ('PENDIENTE','EN_PROCESO')),0) AS total_saldo_pendiente
                FROM sms_cola_envios;
                """
            )
            metricas = cur.fetchone() or {"pendientes": 0, "en_proceso": 0, "enviados": 0, "fallidos": 0, "simulados": 0, "total_saldo_pendiente": 0}
            cur.execute(
                """
                SELECT id,identificacion,nombre,conjunto_residencial,torre_apto,telefono,
                       saldo_calculado,mensaje_texto,tipo_campana,estado,fecha_creacion,
                       fecha_proceso,fecha_envio,error_detalle
                FROM sms_cola_envios ORDER BY id DESC LIMIT 50;
                """
            )
            cola = cur.fetchall()
            ...
            cur.execute(
                ...
                FROM inmuebles_ph i JOIN expensas_ph e ON e.inmueble_id=i.id
    ...
    try:
        ...
        with conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                row_t = cur.fetchone()
                template = row_t["cuerpo_template"] if row_t else "{nombre}, mora en {conjunto} {unidad}. WhatsApp: {telefono_wa}"
                ...
                for d in deudores:
                    ...
                    cur.execute(
                        ...
                        INSERT INTO sms_cola_envios(
                            inmueble_id,contacto_id,identificacion,nombre,conjunto_residencial,
                            torre_apto,telefono,saldo_calculado,mensaje_texto,tipo_campana,estado
                        ...
                        """,
                        (
                            d["inmueble_id"], d.get("contacto_id"), d["identificacion"], nombre_corto,
                            conjunto, unidad, tel, d["saldo_total"], msg, tipo_campana,
                        ),
                    )
                    insertados += cur.rowcount
        return RedirectResponse(url=f"/sms?mensaje=Se+cargaron+{insertados}+mensajes+nuevos+en+cola", status_code=303)
    except Exception as exc:
...
def despachar_campana(
    ...
    if not ANDROID_GATEWAY_URL and not dry_run:
        return RedirectResponse(url="/sms?error=ANDROID_GATEWAY_URL+no+esta+configurada.", status_code=303)
    background_tasks.add_task(_tarea_despacho_background, limite_acotado, dry_run)
    modo = "Simulacion" if dry_run else "Despacho+Gateway"
    return RedirectResponse(url=f"/sms?mensaje={modo}+iniciado", status_code=303)
...
def api_obtener_pendientes(
    ...
    es_habil, motivo = validar_horario_ley_2300()
    if not es_habil:
        return JSONResponse({"status": "bloqueado_horario", "motivo": motivo, "mensajes": []})
    ...
    try:
        ...
        return JSONResponse({"status": "ok", "cantidad": len(mensajes), "mensajes": mensajes})
    finally:
        conn.release()


@router.post("/api/reportar")
def api_reportar_envio(data: ReporteLote, authorization: Optional[str] = Header(None)):
    _autenticar_bearer(authorization)
    actualizados = 0
    rechazados = 0
    for rep in data.reportes:
        ...
        if confirmado:
            ...
            rechazados += 1
    return JSONResponse({"status": "ok", "actualizados": actualizados, "rechazados": rechazados})
